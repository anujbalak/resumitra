...
function Resumes({
  ...
  function newResumeSubmit(e) {
    ...
    const resume = {
      name: e.target.childNodes[0].childNodes[1].value,
  ...
  function deleteResumeHandler(e) {
    ...
    saveInLocalStorage(
      resumeList.filter((resume) => resume.id !== deletedResumeId),
    );
  ...
  return (
    <div className="resumes">
      ...
      <RenderResumes
        ...
        onEditResume={onEditResume}
      />
      <Button name={"New Resume"} btnEventHandler={btnEventHandler} />
    </div>
  );
}

function NewResumeDialog({ newResume, newResumeSubmit }) {
  if (newResume === true) {
    return (
      <dialog className="dialog" open>
        <form method="dialog" onSubmit={newResumeSubmit}>
          ...
          <p className="hint">
            you can enter the post for which you are making
          </p>
          <Button name={"Submit"} />
        </form>
      </dialog>
...
function RenderResumes({ resumeList, deleteResumeHandler, onEditResume }) {
